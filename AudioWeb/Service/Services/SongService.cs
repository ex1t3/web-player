...
namespace Service.Services
{
  public class SongService
  {
    ...
    {
      var domains = new string[] { ".me", ".org", ".ru", ".ua" , ".com", ".net", ".fm", ".top", ".music" }; // Need to be improved
      var symbols = new string[] { " ", "[", "(" };
      var isSymbolMatched = false;
      foreach (var domain in domains)
      {
        if (song.Name.ToLower().Contains(domain))
        {
        ...
        if (song.Artist.ToLower().Contains(domain))
        {
          foreach (var symbol in symbols)
          {
        ...
        if (song.Album.ToLower().Contains(domain))
        {
          foreach (var symbol in symbols)
          {
            var index = song.Album.LastIndexOf(symbol);
            if (index == -1) continue;
            song.Album = song.Album.Substring(0, index);
            isSymbolMatched = true;
          }
        ...
        if (!song.Genre.ToLower().Contains(domain)) continue;
        {
          foreach (var symbol in symbols)
          {
            var index = song.Album.LastIndexOf(symbol);
    ...
    {
      return await _db.Songs.OrderByDescending(x => x.Activity).Take(100).ToListAsync();
    }
    ...
    public async Task<List<Song>> GetSongsOfGenre(string genre)
    {
      return _dbSong.GetMany(x => x.Genre.Contains(genre)).ToList();
    }
    ...
    public async Task<List<string>> GetGenresList()
    {
      var random = new Random();
      var specialChars = ",()/|01234.\\".ToCharArray();
      var genresList = _db.Songs.Select(x => x.Genre).Where(x => x.Length >= 3).Distinct().ToList();
      var length = genresList.Count;
      while (length > 1)
      {
        var k = random.Next(length--);
        var temp = genresList[length];
        if (temp.IndexOfAny(specialChars) != -1)
        {
          length--;
          genresList.Remove(temp);
          continue;
        }
        genresList[length] = genresList[k];
        genresList[k] = temp;
      }

      return genresList.Take(10).ToList();
    }
  }
}
