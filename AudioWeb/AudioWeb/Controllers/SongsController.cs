...
namespace AudioWeb.Controllers
{
  ...
  public class SongsController : ApiController
  {
    ...
    {
      var userId = _userService.GetUserByIdentityName(User.Identity.Name).Id;
      ...
      var image = GetResizedImage(tags);
      var imageLink = GetBase64String(image);
    ...
    [HttpGet]
    [Route("GetGenresList")]
    public async Task<IHttpActionResult> GetGenresList() // Method used for retrieving genres list
    {
      var result = await _songService.GetGenresList();
      return Json(result);
    }

    [HttpPost]
    [Route("GetSongsOfCertainGenre")]
    public async Task<IHttpActionResult> GetSongsOfCertainGenre([FromBody]string genre) // Method used for retrieving list of songs of certain genre
    {
      var result = await _songService.GetSongsOfGenre(genre);
      return Json(result);
    }

    [HttpPost]
    [Route("GetSongsOfArtist")]
    public async Task<IHttpActionResult> GetSongsOfArtist([FromBody]string artist) // Method used for searching similar songs or artists to user's typed text
    {
