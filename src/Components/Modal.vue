  ...
  <div v-bind:class="{active: isModalVisible}" class="modal-overlay">
    <div v-bind:class="{active: isModalVisible}" class="modal">
      ...
      <div v-if="currentContentType === 1" class="modal-content">
        <h4 class="centered">Choose new name for</h4>
        <div class="input-group">
          <input v-model="playlistName" ref="playlistName" name="playlistName" placeholder="Playlist"
            class="input-field" type="text" required>
        </div>
        <div class="input-group centered">
          ...
          <button @click="{}" class="button-form" type="button"><i class="far fa-trash-alt"></i> DEL</button>
        </div>
      </div>
      <!-- content -->
    </div>
    ...
    editPlaylist() {
      if (this.checkPlaylistValidity()) {
        ...
        this.$root.$emit(
          "notificate",
          "success",
          "'" + oldName + "'" + ' changed to ' + "'" + this.playlistName + "'",
          3000
        )
      }
    },
